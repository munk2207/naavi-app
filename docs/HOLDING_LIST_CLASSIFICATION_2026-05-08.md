...
Four lists, each with the same column shape (`ID | Description | Notes | Server/AAB`):
...
| B2b | Voice "Naavi stop" stop-word regression | Stop-word matcher (`naavi-voice-server/src/index.js` line 5161) compares exact strings. 'Naavi stop' lower-cased = 'naavi stop' which isn't in the array. Bare 'stop' / 'enough' / 'got it' still work. Fix: strip leading wake-word (`/^(na+h?v+ee?|naavi|navi)\s+/i`) before the stopWords lookup. Optional follow-up: also fire on interim transcripts (not just FINAL) for faster cut-off; track separately if mid-sentence interrupt feels laggy after the fix. ~30 min, server-only. | Server |
| B2c | Voice Deepgram first-word truncation on barge-in | Deepgram drops first word on barge-in (`'What time is it?'` → `'Time is it?'`). Trivial fast-path regex (`naavi-voice-server/src/index.js` line 1343) doesn't cover leading-word-clipped variants for time/date/day. Fix: add `(?:what\s+)?time\s+is\s+it`, `(?:what\s+)?day\s+is\s+it`, `(?:what\s+)?(?:date\|today's\s+date)` alternations. ~15 min, server-only. Deeper fixes (audio prebuffer / Deepgram endpointing tuning / STT retry) deferred — track separately if leading-word loss persists. | Server |
| B2d | Voice name-search mistranscription ("Hussein") | Keyterm system shipped (`fetchKnownNames` from `knowledge_fragments` type=relationship + `user_settings.voice_keyterms` array + hardcoded `naavi`/`nahvee`). Original Apr 21 report predates this fix. **Verify first:** check whether "Hussein" is in Wael's `voice_keyterms` array OR is a relationship-type knowledge fragment; if missing, add via SQL (`UPDATE user_settings SET voice_keyterms=...`); re-test "tell me about Hussein" via voice. If it works → close. If it still mistranscribes despite keyterm priming → escalate to phonetic fallback (Soundex / Metaphone / Levenshtein on transcript at orchestrator layer; days of work). | Server |
