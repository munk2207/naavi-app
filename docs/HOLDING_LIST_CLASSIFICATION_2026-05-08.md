...
| F6a | Mini AI-triage support system (Phase 1 — storage + manual flow) | server | **Foundation seeded 2026-05-20 by the B4j incident** — Hussein reports → Wael relays → Claude investigates → Wael approves → Claude sends. That workflow becomes a real system. **Phase 1 scope:** add `tickets` table (id, ticket_number, created_at, source_channel, user_id, subject, body, status [new/investigating/drafted/approved/sent/closed], linked_holding_id, draft_response, approved_by, approved_at, sent_message_sid, audit_trail jsonb). Add `ingest-ticket` Edge Function that Formspree webhook calls on every form submission — creates a ticket row + sends Wael an SMS notification with ticket #. Manual flow after that: Wael opens Claude, asks "show ticket N", Claude investigates (per the CLAUDE.md "no unverified claims" rule — every claim traces to evidence), drafts response, Wael approves, Claude sends via send-sms / send-user-email. Status transitions logged to audit_trail. **Phase 2** (auto-triage on ingest), **Phase 3** (admin UI), **Phase 4** (pattern detection) deferred — Phase 1 first to prove the data model before automating. Reference the new "no unverified claims" rule + holding-list-as-inventory rule + email routing memory — this system institutionalises all three. ~2 hours code for Phase 1. | Server | open (Phase 1 — design approved, build queued) |
