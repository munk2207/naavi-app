...
| B2c | You can't talk over Naavi at all on the phone (root cause shared with B2b) | voice | When Naavi is reading aloud and you want to cut in with a new question (e.g. *"What time is it?"*) without waiting for her to finish, nothing happens — she keeps reading and your question is never heard. Validated 2026-05-08: tried mid-sentence question while Naavi was talking; she ignored it. **Same root cause as B2b** (stop words don't interrupt either). The whole "talk over Naavi" path is broken — voice isn't listening to user audio while it's playing back its own. Original classification was about a narrower bug (Deepgram dropping the first word). Today's bigger problem is that no user speech reaches the system at all during Naavi's playback. Server-only change. **Fix deferred — tackle B2b and B2c together since they're symptoms of the same broken interrupt path.** | Server |
