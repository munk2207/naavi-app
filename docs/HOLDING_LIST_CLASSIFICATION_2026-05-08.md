...
| F4e | Public demo line Polly Joanna → Cora migration | voice | Shipped 2026-05-18 (commit `1c2db61`), live-tested by Wael same day, **reverted same day** (commit `a0c3597`). Two regressions made Cora unusable on the demo: (1) menu prompt measured 8.6s Deepgram fetch latency (vs Polly's ~instant Twilio-built-in `<Say>`) → unacceptable dead air after user said "yes"; (2) "I heard <name>" prompt sounded different from Cora on the phone despite served audio being Cora end-to-end (Twilio telephony-layer artifact, not reproducible from code). Surgical revert preserved the scenario refresh (Hilton/Toronto for Bills, airport/Sarah for Location). If Cora on the demo ever becomes a real requirement again, the architecture is **pre-baked static MP3s** in `naavi-voice-server/public/audio/` served via `express.static` — Twilio fetches static files in ~200ms vs 8.6s for dynamic Deepgram. Reference memory: `project_naavi_demo_iheard_voice_deviation.md`. | Server | deleted: reverted same day (2026-05-18) — 8.6s Deepgram latency + voice deviation on phone, unusable on first-impression surface |
