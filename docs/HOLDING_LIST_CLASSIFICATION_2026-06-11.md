...
| ------- | ---------------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------- |
| B2m | Parity audit — mobile vs voice gaps | both | Full gap list in `docs/MOBILE_VS_VOICE_PARITY_AUDIT_2026-06-12.md`. Open gaps: (1) Re-arm expired location alert — voice only (High); (2) SPEND_SUMMARY not in voice executeAction (Medium); (3) LOG_CONCERN / UPDATE_PROFILE not in voice executeAction (Medium); (4) UPDATE_MORNING_CALL not wired in mobile (Medium); (5) Contact lookup by phone not enriched on voice (Low); (6) Verified-address gate missing for FETCH_TRAVEL_TIME on voice (Low); (7) Source-hint filtering not passed on voice global search (Low). | Both | open — see parity audit doc |
...
| ID | Description | Surface | Notes | Server/AAB | Status |
|----|-------------|---------|-------|------------|--------|
| F5c | Executable tasks on alert fire — auto-send messages/calls when location alert triggers | mobile + server | **Observed 2026-06-13 (Wael test):** When a location alert is set with tasks like "text Ahmed that you'll be there by 9 AM" and "call Sarah about the proposal", Naavi stores them as reminder text strings — not automated actions. On arrival, Naavi surfaces the notes to the user as reminders; no messages are sent automatically. **Expected behavior:** tasks attached to an alert should be auto-executed when the alert fires (send SMS/WhatsApp to Ahmed, trigger a call or notification to Sarah). **Required work:** (1) At alert-set time: resolve recipient names to phone numbers via contact lookup and store as structured action objects in `action_config.tasks[]` (not plain strings); (2) `evaluate-rules` must execute each task action (send-sms / send-whatsapp / voice call) when the location trigger fires, in addition to firing the user's own arrival notification; (3) Mobile UI — task cards need to distinguish between "reminder note" (text only) and "automated action" (will auto-execute). **Complexity: HIGH** — needs contact resolution at alert-set time, structured task schema change, evaluate-rules executor extension. Design session required before code. | Both | open — design session required |
| F5d | Outbound call to third party — "Call Sarah at +16137697957 and say hi" | Both | Naavi dials a third-party number via Twilio, plays a TTS message on the user's behalf, and hangs up. Mobile: DRAFT_MESSAGE-style confirm card before dialing. Voice: speak the intent + confirm. Requires new action type (e.g. `MAKE_CALL`), a new Edge Function (`outbound-call`) that uses Twilio REST API to initiate the call, and TTS for the spoken message body. Low complexity — Twilio REST outbound calls are straightforward. No AAB needed for the Edge Function; mobile card needs AAB. **Added 2026-06-14 (Wael).** | Both | open |
...
| ID     | Description                                                                     | Surface | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Server/AAB | Status                                              |
| ------ | ------------------------------------------------------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------------------------------------------------- |
| ARCH-1 | Deterministic-first architecture — Layer 2 intent gate + voice regression suite | backend | **Design decided 2026-05-29.** Three-layer model: (1) Pre-Claude bypass for known intents (Layer 1 — FAST_CHAT_RE + LIST_CONNECTION_RE — **SHIPPED**); (2) Intent verification gate — Claude classifies intent into structured object {type, confidence, params}, server routes: high confidence + handler → deterministic execution, low confidence → ask Robert to confirm, out of scope → honest-out (Layer 2 classification infrastructure — **SHIPPED** as `classification.level` A/B/action/chat in `naavi-chat`; deterministic handlers for LIST_READ, REMINDER_READ, MEMORY_SEARCH, PERSON_LOOKUP, CREATE_TICKET, GMAIL_SEARCH — **SHIPPED** including null-safety + body fallback + from_email routing + trash-email filtering fixes shipped 2026-06-13 build 252); (3) Path B transparent best-effort disclosure — **SHIPPED** 2026-05-29. **Side effect found 2026-06-13:** Path B was firing on simple time/date questions (classified Level B instead of bypassed). Fixed by expanding FAST_CHAT_RE to catch "what is the time now" / "what is today date" variants. **2026-06-14 ARCH-1 continuation:** READ_CALENDAR handler added — "what do I have today", "what's coming up", "do I have anything tomorrow", "when's my next meeting" now bypass Claude entirely via expanded B6e regex + new Level A READ_CALENDAR intent in Haiku classifier. 275/275 tests green. Deployed. **Remaining work:** multi-action "." separator bug (Naavi stops after first action when user separates tasks with periods instead of "and"); "Call Jasmine" spurious calendar event on birthday-reminder flow; T3c voice regression suite. | Both       | open — multi-action separator + T3c remaining |
