...
| ID | Description | Surface | Notes | Server/AAB | Status |
|----|-------------|---------|-------|------------|--------|
| F5c | Executable tasks on alert fire — auto-send messages/calls when location alert triggers | mobile + server | **CLOSED 2026-06-15** — confirmed fully shipped and tested by Wael. Two paths: (1) Location alerts: contacts pre-resolved at set-time via `resolveTaskActions()`, stored in `action_config.task_actions[]`. (2) Time/other alerts: Claude emits structured task objects into `tasks[]`; `evaluate-rules` resolves contact at fire-time via `lookup-contact` (lines 892-918) then sends SMS/email. Both paths confirmed working. Original audit entry was stale. | Both | **CLOSED 2026-06-15** |
