...
**Architecture Version:** 2026.09.05.18 (date-and-revision format — avoids the ambiguity of a bare "latest Architecture Reference" reference elsewhere in the governance doc).

**Revision 18 (2026-09-05, [[F25]] Stage 2 Phase 8):** the mobile half of the FAQ. The app carried its own copy of **12** questions against **26** published, with a matcher that could not clear its own threshold on one word and returned nothing at all under three words — and its header instructed a human to keep it in sync while nothing enforced that. `lib/faq.ts` is deleted; both support screens call `match-faq`. **§5a's Priority 1d row closes: two copies become one.** ⭐ **The finding this revision exists to record is the one this document did NOT have.** Revision 17 documented `lib/faq.ts` as a surviving *copy of the content*. It did not record that mobile ran an **independent matcher with different semantics** — a three-word minimum and a threshold no single word could clear. A reader consulting this document would have concluded the app merely lacked answers, when it also matched by a different rule. **Outcome 3 under the Architecture Drift Rule, recorded at Phase 1A and carried here under v4.3; Wael approved recording the matcher, not merely the copy, on 2026-09-04.** **Not an ownership change** — `match-faq` already owned matching and still does; mobile became a second consumer, so §4's Ownership Change Rule did not apply, unlike Stage 1. **§2's row also gains what the rate limiter counts:** the subject is the signed-in user where there is one and the address otherwise, because mobile arrives behind carrier NAT where one address is shared by many people. Two pre-existing defects were fixed alongside, both approved at Phase 3: the limiter **failed open silently** — both database errors discarded, so a failed read produced `used = 0` and the ceiling never tripped — and its counter **lost updates under concurrency**, the same read-modify-write shape §2c records for voice-PIN. **Both measured after the fix:** ten concurrent calls returned 1 through 10, each distinct. Bumped in the same commit as the edits.
...
| **FAQ — matching a described problem to an answer** | `match-faq` (Shared Core) | Unauthenticated. **Selects from published answers and never composes one** — the model is given a slug list and every slug it returns is validated against that set, so an invented answer is structurally impossible rather than merely instructed against. **Four consumers since [[F25]] Stage 2 (2026-09-05):** the website's two support forms and the app's two, `app/contact.tsx` and `app/report.tsx`. **The app previously ran its own matcher**, `lib/faq.ts`, now deleted — see §5a Priority 1d. **Rate-limited per SUBJECT, not per address:** the signed-in user where a bearer token verifies, the IP otherwise. Mobile arrives behind carrier NAT, where one address is shared by many customers. ⚠️ **The anon key resolves to NO identity** — byte-identical on every install, so counting it would put every signed-out app user in one bucket, worse than the address. ⚠️ **Must stay deployed with `--no-verify-jwt`.** It reads an Authorization header when present but must remain callable without one: `mynaavi.com/report` and `/contact` send no credentials, and gateway JWT verification would 401 them **before this function runs**. |
...
| **Immediate draft (`DRAFT_MESSAGE`)** | **No shared resolver.** The tool schema instructs Claude to emit a bare name and assigns resolution to the orchestrator, so the **mobile card resolves it at display time**. **Self-references are resolved in Shared Core by `resolveSelfRecipient()`, awaited at both construction sites** — a test asserts exactly two call sites. Everything else is still client-side | **Partly** — self only |
...
**Priority 1d — FAQ content and matching: CLOSED 2026-09-05.** Until [[F25]] Stage 1 the same answers were authored as markup in three places — `mynaavi-website/faq.html` as visible HTML, the same file again as a hidden search-engine block, and `lib/faq.ts` as a TypeScript keyword table. **The third had been eleven questions behind for months**, and its own header instructed a human to keep it in sync — the exact "knowledge recorded with nothing enforcing it" pattern this document names about itself. Stage 1 collapsed the first two into one database record generated at build time. **Stage 2 deleted the third (2026-09-05), and with it a second thing this document had never recorded: mobile was not merely holding a stale COPY, it was running an INDEPENDENT MATCHER with different semantics** — a three-word minimum (`lib/faq.ts:118`) and a score threshold no single word could clear (`:116`), so `"how do i delete"` found nothing there while the website answered it. **One record, one matcher, four consumers.** The staged deferral this row described is discharged; the decision point it named — whether the app would call `match-faq` or generate its own copy — was answered by Wael on 2026-09-04 in favour of calling it.
...
| **FAQ content and matching** — Priority 1d (**CLOSED 2026-09-05**) | ✅ | | **Was three copies, then two, now one.** Website HTML, its hidden search-engine block, and `lib/faq.ts` — the third **eleven questions behind** for months. Stage 1 generated the first two from one record; **Stage 2 deleted the third**. The row also understated the problem while it was open: mobile ran an **independent matcher**, not just a stale copy. `match-faq` is now the single matcher for all four support surfaces |
...
| **`DRAFT_MESSAGE` construction, intra-`naavi-chat`** (**new row 2026-09-01**) | | ✅ | **Not** an accepted Exception. Two independent construction sites inside one function — Layer 2 (`:2181`) and the Path B tool-use mapper — mirroring the location-alert shape §2e documents. A test asserts `resolveSelfRecipient` is awaited at **exactly two** call sites, because [[B9x]] lost three live staging trials to a fix placed on one site alone. Added by [[B11l]] Phase 8 |
