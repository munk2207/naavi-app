-- T4 — STEP 2 of 2.  Run this in the PRODUCTION SQL editor.
--
-- This is the migration Wael authorized. Unlike Step 1, it runs REAL commands.
--
-- But every one of them tells production something it already knows. All 32
-- were checked one by one against production own catalogue before this file
-- was written: each NOT NULL is already NOT NULL there, each default is
-- already that default, and morning_call_phone already has no default. So the
-- commands run, and nothing about production changes.
--
-- The point is not to change production. It is to make the migration FILES
-- describe production truthfully, so staging and every future environment can
-- be rebuilt from them correctly. Staging received this same file and it
-- changed 30 things there.
--
-- IF SOMETHING IS UNEXPECTED, THIS STOPS. The 19 checks at the top count rows
-- that would block each change. If any is found, the whole thing aborts with a
-- message and nothing is applied. That is deliberate: it stops rather than
-- guesses.
--
-- ══════════════════════════════════════════════════════════════════════
...
VALUES ('20260820000000') ON CONFLICT (version) DO NOTHING;

-- Confirmation. Should read 82.
