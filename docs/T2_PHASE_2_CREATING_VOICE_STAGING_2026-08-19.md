...
- **Introduces new duplication?** **NO** — deliberately avoided via the single guard module. One unavoidable exception: B10 cannot import the Deno `_shared` module into the Node voice server, so a small equivalent check exists there. **This is a second implementation of the same rule and must be declared.** Phase 3 should decide whether it warrants an Architecture Exception (§5a) or is acceptable as a runtime-boundary necessity.
