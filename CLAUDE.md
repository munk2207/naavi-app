...
15. **SUSPENDED 2026-05-16 — auto-tester is DISABLED.** Previously: `npm run test:auto` MUST BE FULLY GREEN BEFORE EVERY AAB BUILD (Wael 2026-05-01 after V57.9.8 first 44/44 green). Suspended after two destructive-write incidents against the real test user (mynaavi2207@gmail.com): calendar floods and `user_settings.phone` NULL'd by `multi-phone.ts::clearTestUserPhones`. Both have code fixes shipped (V57.16 calendar teardown + V57.16 fixtures snapshot/restore), but Wael paused the runner pending an audit of any other destructive write paths against real-user data. The runner exits immediately unless `AUTO_TESTER_ENABLED=true` is set in env. Until re-enabled, DO NOT skip Rule 15 silently — flag in the AAB-build conversation that this gate is in place and the suite was not run.
