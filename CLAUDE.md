...
**Most recent handoff:** `docs/SESSION_HANDOFF_2026-05-14_V57.15.5_BUILDS_176_177.md` — **READ THIS FIRST**. Session 2026-05-14 shipped V57.15.5 in two builds: (1) **Build 176** (commit `fae265c`) — Voice PIN UI in Mobile Settings (Set/Change/Remove + new `remove` op on `manage-voice-pin` Edge Function), multi-phone refinements (#4 pretty-print +1 NA numbers, #5 tap-to-edit-in-place backup rows, #7 cap at 5 phones), Lists testIDs replacing the V57.15.4 Maestro regex workaround. (2) **Build 177** (commit `5ce56ad`) — direct response to live-test findings on build 176: PIN modal `KeyboardAvoidingView` wrap (turned out structurally insufficient), Primary phone tap-to-edit + pretty-print parallel to backup pattern, `onSubmitEditing` wiring on inline-edit + add-backup + PIN confirm fields (keypad Done commits without chasing on-screen ✓), verified-address rejection messages now name the place (#24 closed). Auto-tester 108/108 green at both builds. Live test on Wael's phone: Tests 1, 2, 5, 7 PASS clean; Tests 3, 4 PASS via blind-type + keypad Done workaround. Prior handoff: `docs/SESSION_HANDOFF_2026-05-13_V57.15.4_F1A_VERIFIED_F1D_CLOSED.md`.
...
4. ~~V57.15.5 build 176 + 177~~ — **SHIPPED 2026-05-14.** Caller PIN feature mobile-complete end-to-end (UI + Edge Function + voice-server flow + prompt RULE 19 all live).

5. ~~Verified-address rejection naming (#24)~~ — **CLOSED 2026-05-14** via build 177 (commits at `hooks/useOrchestrator.ts:830` + `:924`).

6. ~~Haptic VIBRATE permission + duration (#22)~~ — **CLOSED 2026-05-14** as phantom — already shipped in V57.11.7 (long-press mic vibration at 150ms).
...
**Last AAB on Wael's phone:** V57.15.5 build 177 (commit `5ce56ad`), installed 2026-05-14. Second phone has V57.14.3 build 169 (from 2026-05-11).
...
**Auto-tester (latest):** 108 ✓ / 0 ✗ / 0 errored / 0 skipped. Run with `npm run test:auto`. Includes `prompt-regression` (15), `truth-at-user-layer` (1, retry-on-flake), `list-connections` (10), `hosted-replies` (5), `pending-dwell` (5), `data-integrity` (3), `source-intent` (5), `brief-unread` (2), `search-normalization` (4), `gmail-freshness` (1), `lists` (4), `voice-pin` (7), `multi-phone` (4), `lists-reconcile` (2), `multiuser` (20). Re-confirmed green at builds 176 + 177 commits. Retry-on-flake covers `chat`, `smoke`, `prompt-regression`, `truth-at-user-layer` (all Haiku-driven categories).
...
18c. ⭐ **V57.15.6 build 178 — TOP PRIORITY.** Three bugs caught during build 177 live test: (a) PIN modal KAV restructure (KAV outermost + flex-start anchor — bulletproofs against any keyboard quirk; unblocks Tests 3+4 visual UX), (b) Primary edit screen-shift (same keyboard family), (c) Multi-phone auto-persist on +/X/edit — remove "Save phones" button entirely (root-cause UX bug — users tap + expecting persistence, navigate away, entry lost). ~1 hour code + retest. Plan locked in 2026-05-14 handoff.
19. Multi-phone identity (`phone_numbers[]` schema + Settings UI) — **partially shipped** in V57.15.0 (Wave 2 Phase E) + V57.15.5 (refinements #4 #5 #7). Auto-persist redesign in build 178 (item 18c).
