...
**Most recent handoff:** `docs/SESSION_HANDOFF_2026-05-13_V57.15.4_F1A_VERIFIED_F1D_CLOSED.md` — **READ THIS FIRST**. Session 2026-05-13 shipped: (1) **F1a Wave 2 V57.15.4 build 175** — newline formatter for multi-item LIST_CONNECTION_QUERY answers (escapes Samsung Yoga truncation zone by always spanning 3+ visible lines) + tappable list rows in the query card. Live-verified end-to-end on Wael's phone — asked *"what lists are on my Costco alert?"* → got perfect newline-formatted answer + tappable card with grocery and cottage rows → tap navigates to list-detail. (2) **F1d Tests 3 + 4 PASS live** on Twilio voice call — recursive mute keeps offer pending across multiple privacy-mute fires, 30-sec silence auto-discards without false-positive delivery. F1d feature now CLOSED. (3) **4 voice-server fixes from F1d live test:** privacy-mute regex relaxed for Deepgram phonetic confusables ("sound"↔"pound", "quiet"↔"quite") + aggregated-text check at UtteranceEnd for sentence-split utterances, SMS confirmation TTS clearer ("Sent." → "SMS sent to your phone."), idle-prompt suppressed during pending hosted-reply. (4) **Prompt v74 deployed** — fixes F1a entityType inconsistency (CLAUDE.md item 16a CLOSED) with mandatory-field callout + entityType-inference rules per phrasing pattern. (5) **Maestro flows 14/15/16/17 all green** after fixing regex-anchor on tab assertion + scrollUntilVisible for Settings + longer cold-load timeouts on Lists screen. Auto-tester 107 → 108 green (new entityType regression test). Prior handoff: `docs/SESSION_HANDOFF_2026-05-12_V57.14.4_F1A_WAVE_1.md` (F1a Wave 1 voice surface end-to-end).

**Top of next session — PRIORITY ORDER (Wael 2026-05-13):**

1. **V57.15.5 AAB — Caller PIN + Lists testIDs.** ⭐ Top priority, ~1.5 hours. Per Wael's earlier decision (`project_naavi_caller_pin_chosen_over_biometric.md`), the caller PIN feature replaces Picovoice Eagle permanently — no more vendor-approval waiting. **Phase A** (~1 hour): DB migration (`user_settings.caller_pin_hash` text + `caller_pin_set_at` timestamptz, nullable) + Mobile Settings UI section for PIN set/clear + voice-server PIN-prompt flow on caller-phone-lookup miss + prompt v75 addition for PIN vocabulary. The `manage-voice-pin` Edge Function already exists (commit `9893c5f`) with 7 auto-tester cases passing. **Phase B** (~15 min): Add `testID={'lists-tab-' + key}` to each tab `TouchableOpacity` in `app/lists.tsx:156` + update `e2e/14-lists-screen-tabs.yaml` to use `id:` selectors instead of the `.*` regex workaround we shipped today. **Phase C** (~15 min): version bump 175→176 / V57.15.4→V57.15.5 + `test:auto` green + AAB with `--auto-submit`. One AAB, two real user-visible improvements. **Recommend A+B+C as one continuous push.**

2. **Geofence reliability — STILL BLOCKED on vendor replies.** Transistorsoft email sent 2026-05-12 (no reply); **follow-up email DRAFTED in 2026-05-13 chat log** (not sent — Wael's call when to send). Radar email also sent 2026-05-12, no reply. **Do NOT pay the $350 Transistorsoft license without trial verification first.** Honest risk assessment captured in 2026-05-13 chat: Transistorsoft is significantly less risky than Radar for our use case (bounded financial exposure, no privacy compromise via on-device-only execution, no vendor lock-in, no recurring cost). When trial key arrives → install + drive-test on Samsung phone → pay only if reliable. If still no reply by next session, send the drafted follow-up first thing.

3. **Server-side fast wins (alternative path if AAB skipped):** `naavi-spend-summary` Edge Function (~1 hour, approved 2026-04-30 but never built) + Voice live-calendar fetch (~30 min, voice still on stale snapshot vs mobile V57.11.6) + `resolve-place` radius 100→500 + address routing fix (~30 min). Three real user-visible improvements, zero AAB.

4. ~~F1a entityType inconsistency~~ — **CLOSED 2026-05-13** via prompt v74.

5. ~~F1d edge-case live tests (Tests 3 + 4)~~ — **CLOSED 2026-05-13** both PASS.
...
**Last AAB on Wael's phone:** V57.15.4 build 175 (commit `a9603bd`), installed 2026-05-13. Second phone has V57.14.3 build 169 (from 2026-05-11).
**Last AAB on Robert's phone:** V56.6 (build 115), installed 2026-04-28. **Do NOT promote V57.x to Robert until geofence reliability is solved (still blocked on Transistorsoft / Radar reply — see priority #2).**

**Auto-tester (latest):** 108 ✓ / 0 ✗ / 0 errored / 0 skipped. Run with `npm run test:auto`. Includes `prompt-regression` (15 — 1 new entityType-required test added 2026-05-13), `truth-at-user-layer` (1, retry-on-flake), `list-connections` (10), `hosted-replies` (5), `pending-dwell` (5), `data-integrity` (3), `source-intent` (5), `brief-unread` (2), `search-normalization` (4), `gmail-freshness` (1), `lists` (4), `voice-pin` (7), `multi-phone` (4), `lists-reconcile` (2), `multiuser` (20). Retry-on-flake covers `chat`, `smoke`, `prompt-regression`, `truth-at-user-layer` (all Haiku-driven categories).
...
18a. ~~**F1a Wave 2 (V57.15.0)**~~ — **SHIPPED 2026-05-12 (V57.15.0) + follow-ups 2026-05-13 (V57.15.4).** All 4 phases (A orchestrator handlers, B Lists screen + list-detail, C alert-detail connection card with X, D migration) shipped in V57.15.0 build 171 (commit `a705196`). V57.15.4 build 175 (commit `a9603bd`) added: newline formatter for multi-item LIST_CONNECTION_QUERY answers (truncation fix) + tappable list rows in the query card. Live verified 2026-05-13.
18b. ⭐ **V57.15.5 (Caller PIN + Lists testIDs)** — **TOP PRIORITY**. Caller PIN replaces Picovoice Eagle permanently per `project_naavi_caller_pin_chosen_over_biometric.md`. Phase A: DB migration + voice-server PIN-prompt flow + Mobile Settings UI section + prompt v75 (~1 hour). Phase B: testIDs on Lists tabs (~15 min) closes today's Maestro regex workaround. Phase C: AAB V57.15.5 (~15 min). Plan locked in 2026-05-13 handoff.
19. Multi-phone identity (`additional_phones[]` schema + Settings UI) — **partially shipped** in V57.15.0 (Wave 2 Phase E). Refinements queued.
