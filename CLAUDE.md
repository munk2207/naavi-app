...
**Most recent handoff:** `docs/SESSION_HANDOFF_2026-05-12_V57.14.4_F1A_WAVE_1.md` — **READ THIS FIRST**. Session 2026-05-12 shipped: (1) **F1a Session 2 Wave 1 — voice surface end-to-end** (resolve-entity-ref Edge Function + 4 voice-server `executeAction` handlers + missing tool definitions + speech defaults + query-result formatter + "attached/detached" vocab via prompt v69). Voice users can now connect/disconnect/query/delete lists wired to entities via Twilio call; verified live (cottage list → 688 Bayview office rule). 6 new prompt-regression tests; auto-tester 85 → 91 green. (2) **Geofence diagnostic — manual polling plan disproven**. V57.14.4 build 170 added FG-service heartbeat log + Supabase mirror; data showed FG service receives ZERO location updates from Android after backgrounding AND cannot be restarted from background due to Android 12+ FGS restriction. Manual-polling-on-FG-service plan abandoned. Vendor evaluation underway: emails sent to Transistorsoft + Radar; awaiting replies. **Decision parked:** geofence work BLOCKED on vendor reply; next priority pivots to F1a Wave 2. Prior handoff: `docs/SESSION_HANDOFF_2026-05-11_V57.14.3_BUILD_169.md` (V57.14.3 build 169 shipping).

**Top of next session — PRIORITY ORDER (Wael 2026-05-12):**

1. **F1a Wave 2 (V57.15.0 AAB)** — ⭐ now the unblocked top priority. Plan locked at 2026-05-12 session close. Four phases: (A) mobile `useOrchestrator.commitPending` handlers for `LIST_CONNECT` / `LIST_DISCONNECT` / `LIST_CONNECTION_QUERY` / `LIST_DELETE` mirroring voice-server logic. (B) New `app/lists.tsx` screen with subcategory tabs **All / Connected / Standalone** + 3-dots-menu entry between Alerts and Notes; new `app/lists/[id].tsx` list-detail with "Attached to:" header. (C) Alert-detail update — connected-list card with delete-connection X icon. (D) Drop legacy `action_config.tasks` + `action_config.list_name` columns (0 legacy rows so data migration is no-op); bump versionCode 170→171, version V57.14.4→V57.15.0; `test:auto` green; AAB with `--auto-submit`. **Recommend doing A+B+C as one continuous push, then D ships them all in one AAB.**

2. **Geofence reliability — BLOCKED on vendor replies.** Two emails sent 2026-05-12: Transistorsoft (`info@transistorsoft.com`) — trial key + refund + Samsung specifics; Radar sales — Samsung One UI handling + pricing for low-MTU tier + trial. **Do not start integration on either until reply is in hand.** Both candidates evaluated; whoever answers the Samsung question with technical specifics (vs hand-waves) wins. The prior session's "manual polling on FG service" plan is dead — V57.14.4 heartbeat data confirmed FG service receives zero location updates from Android on backgrounded Samsung devices AND cannot be restarted from background per Android 12+.

3. **F1a entityType inconsistency (CLAUDE.md item 16a)** — server-side fix; revisit after Wave 2 lands so we can validate fix on both surfaces simultaneously.

4. **F1d edge-case live tests** — Test 3 (recursive mute during offer) + Test 4 (30-sec timeout). Voice call only, no AAB. Unchanged from prior handoff.
...
**Last AAB on Wael's phone:** V57.14.4 build 170 (commit `88e669b`), installed 2026-05-12. Second phone has V57.14.3 build 169.
**Last AAB on Robert's phone:** V56.6 (build 115), installed 2026-04-28. **Do NOT promote V57.x to Robert until geofence reliability is solved (now blocked on Transistorsoft / Radar reply — see priority #2).**

**Auto-tester (latest):** 91 ✓ / 0 ✗ / 0 errored / 0 skipped. Run with `npm run test:auto`. Includes `prompt-regression` (14 — 6 new F1a tests added 2026-05-12), `truth-at-user-layer` (1, retry-on-flake), `list-connections` (7), `hosted-replies` (5), `pending-dwell` (5), `data-integrity` (3), `source-intent` (5), `brief-unread` (2), `search-normalization` (4), `gmail-freshness` (1), `lists` (4). Multi-user matrix in `tests/catalogue/multiuser.ts`. Retry-on-flake covers `chat`, `smoke`, `prompt-regression`, `truth-at-user-layer` (all Haiku-driven categories).

**Current Claude prompt version:** `2026-05-12-v69-attached-detached-vocab` (via `get-naavi-prompt` Edge Function). 2026-05-12 shipped v68 → v69 in sequence (vocab swap "Connected/Disconnected" → "Attached/Detached"; tightened spec-correct WAIT-for-yes adherence as side-effect).
...
4. **Geofence reliability — VENDOR EVALUATION 2026-05-12.** V57.14.4 build 170 added a heartbeat log to the FG location task; data confirmed FG service receives ZERO location updates from Android after backgrounding AND Android 12+ blocks restart from background (`"Foreground service cannot be started when the application is in the background"`). The prior plan to switch to manual polling on FG service location updates is therefore impossible — there are no location updates to poll. Pivoted to third-party SDK evaluation. **Two emails sent 2026-05-12** (text drafts in handoff doc): (a) Transistorsoft `info@transistorsoft.com` — trial key + refund policy + Samsung One UI specifics for `react-native-background-geolocation` ($399 STARTER perpetual); (b) Radar sales — Samsung One UI handling + pricing for low-MTU tier + trial. **Whichever answers the Samsung question with technical specifics (vs hand-waves) wins.** Do not start integration on either until reply is in hand. Memory: `project_naavi_geofence_reliability_open.md` needs update to reflect this pivot.
...
16a. **F1a voice — `entityType` inconsistency on `list_connect`.** Wael 2026-05-12: Claude inconsistently emits the `entityType` field on `list_connect` tool calls (saw it omitted on 2 of 4 attempts during the cottage→688 Bayview test). When omitted, voice-server `executeAction` rejects silently per current required-fields check, leading to "I'll attach..." speech with no actual connection. **Fix combo:** (a) prompt v70 — tighten RULE 8b to ALWAYS include entityType in list_connect, list_disconnect, list_connection_query (when entity-side); (b) voice-server fallback — when entityType is missing, attempt resolution across all V1-supported types (action_rule first as most common) instead of rejecting. Add prompt-regression test asserting entityType is present in the emitted action shape. Marked as needs-more-tests; revisit after Wave 2 mobile orchestrator lands so the fix can be verified on both surfaces simultaneously.
...
18a. **F1a Wave 2 (V57.15.0)** — ⭐ **TOP PRIORITY**. Mobile orchestrator handlers + Lists screen in 3-dots menu (subcategory tabs All/Connected/Standalone) + list-detail with "Attached to:" header + alert-detail connection card with delete-X. One-shot migration to drop `action_config.tasks` + `action_config.list_name` (0 legacy rows). AAB V57.15.0. Plan locked in 2026-05-12 handoff.
