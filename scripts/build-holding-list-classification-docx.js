...
const bugs = [
  ...
  { id: 'B3c', desc: 'Haptic vibration feels too subtle on Samsung long-press', surface: 'mobile', notes: 'Parked 2026-05-10 — duration bump did NOT solve the bug. Build 166 shipped Vibration.vibrate(80) → Vibration.vibrate(150). On Wael\'s Samsung One UI / Android 14 device: long-press triggers the recording UI (function fires correctly) but produces NO perceptible buzz — both Vibration.vibrate and Haptics.impactAsync(Heavy) silently fail despite OS-level vibration intensity at ~80% and all System vibration toggles ON (Touch interactions, etc.). Suggests an Android 14 / Samsung-specific API issue, not a code-logic issue. Next attempts to try (a pattern instead of a single shot, runtime VIBRATE permission re-check, or a different library like react-native-haptic-feedback) require deeper investigation before another AAB. Keeps the bug active — duration bump was a failed attempt.', sa: 'AAB' },
