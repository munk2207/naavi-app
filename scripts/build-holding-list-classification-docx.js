...
const features = [
  ...
  { id: 'F1d', desc: 'User-controlled mute on PC + Mobile (replaces F1c)', surface: 'both', notes: 'Robert in a taxi, waiting room, or anywhere with company can stop Naavi mid-reply by tapping the screen (mobile) or saying "no sound" / "stop" / similar (phone call). After the mute fires, Naavi offers "Want me to text the rest to your phone?" — Robert can opt in privately to get the content without re-broadcasting. Replaces F1c\'s auto-classification approach which was rejected 2026-05-09: pre-tagging items as "private" forces Robert into a public privacy dialogue ("Naavi thinks this is private — want me to text it?") which itself reveals he has something to hide, and false positives put Robert in socially awkward positions he can\'t gracefully recover from. Reactive user-controlled mute leaves the decision in Robert\'s hands every time. Smaller scope than F1c: hours of work, no schema migration, no Settings UI for privacy categories. Server-only on PC (extends existing barge-in/stop-word handler); mobile already has stop-button infrastructure. Severity 1: privacy is still the biggest UX gap for the older healthy independent adult persona; the implementation is just simpler than F1c\'s bundle.', sa: 'Both' },
  { id: 'F2a', desc: 'Onboarding Review (multi-phone + 7 other gaps)', surface: 'mobile', notes: 'Postponed 2026-05-09 — needs further study before walkthrough. Wael\'s call: bundle is 8 items spanning onboarding doc updates and Settings UI additions; not all 8 have crisp product decisions yet, and the relative priority of each isn\'t clear. Defer to a dedicated session that can step back, look at the onboarding flow end-to-end, and decide which gaps are real vs nice-to-have. Original bundle items: (1) multi-phone — additional_phones[] schema + Settings UI + per-phone SMS verify; (2) voice keyterms capture at setup (ties to B2d); (3) quiet hours field in First-Day Settings (currently checklist-only); (4) verified-address-rule expectation under \'What Naavi Learns\'; (5) consolidated \'data NOT to share\' privacy callout; (6) post-install first-call rehearsal with 5 starter prompts; (7) re-install / new-phone flow guidance; (8) first-week-vs-week-two expectation calibration. Source doc: scripts/build-onboarding-guide-docx.js → docs/MYNAAVI_ONBOARDING_GUIDE.docx. Settings UI additions require AAB; doc is a build-script regen.', sa: 'Both' },
  { id: 'F2b', desc: 'Demo line maturity (richer scenarios + conversion path + telemetry)', surface: 'voice', notes: 'Postponed 2026-05-09 — needs further study before walkthrough. Wael\'s call: F2b is a marketing-and-growth feature with business decisions (what metrics matter most, what conversion funnel to optimize, which scenarios resonate) that need a dedicated session. Defer until the goals are clearer. Original sub-pieces: (1) Telemetry — today everything is console.log(), nothing aggregates. Add events table + dashboard query: total calls, avg scenarios played, % opt-in for SMS, click-through on link, conversion to signup, scenario popularity, drop-off points. ~½ day. (2) Conversion attribution — SMS link mynaavi.com/start doesn\'t track which demo call the lead came from. Add per-call signup token in the SMS link; form captures token; DB join lets us see scenario-to-signup correlations. ~½ day. (3) Scenario richness — current 5 scenarios are fully hardcoded. Add more scenarios (medication scheduling, navigation, recurring delegation), variable data per call, one-level branching with canned follow-up responses. ~1–2 days. Sequencing originally proposed: 1+2 first (measurement infrastructure unlocks decisions); 3 deferred until telemetry says which scenarios engage / fall flat. All server-side, no AAB. Already shipped: 5 canned scenarios, DTMF+speech routing, personalized greeting + name capture, 3-scenario / 5-min cap, personalized SMS recap from +14313006228.', sa: 'Server' },
  { id: 'F2c', desc: 'Walkie-talkie style turn-taking on voice — explicit end-of-message signal', surface: 'voice', notes: 'Postponed 2026-05-09 — needs more thinking before commit. Wael\'s call: F2c is a real structural answer to the "PC always-open channel" problem (vs. MV\'s explicit tap-to-talk), but it forces a new convention on the older healthy independent adult target user, and the marker word still has ambiguity issues. Parallel track decided: continue continuous-improvement on the existing silence-detection architecture — echo cancellation, smarter utterance_end_ms tuning, better noise-vs-speech disambiguation. That\'s the safety net for every user, F2c-or-not. Do NOT bet F2c on fixing B2b / B2c. Those interrupt bugs are largely resolved by the 2026-05-09 queue-drain work; the remaining first-interrupt-miss is phone-side echo cancellation, not a turn-taking architecture issue. Original note (preserved for future session): Today Naavi has to guess when you\'ve finished talking via silence detection. The walkie-talkie idea: borrow the radio convention where each speaker says a clear end-marker word to signal "I\'m done, your turn." Open design question: which end-marker word. "Over" is the standard but appears in everyday speech (Wael agrees not the right word for this case). Alternatives: "Naavi" at sentence end, "go ahead", or a hybrid where the marker is optional and silence-detection still works as fallback. Possibly opt-in mode rather than default. Server-only change, no app build. Significant scope: design (pick end-marker, decide on hybrid vs strict) + implementation + user education.', sa: 'Server' },
...
children.push(bullet('Bugs (B): 8 — B1b, B2a, B2e, B3a, B3b, B3c, B3d, B3e'));
...
children.push(bullet('Total: 30 (26 holding-list + 1 missed item B1c added 2026-05-08 + 1 new feature F2c added 2026-05-08 + 1 new feature F1d added 2026-05-09 superseding F1c + 1 new bug B2e added 2026-05-09)'));
...
children.push(bullet('1 (top): 4 — B1b, F1a + F1d, T1a'));
...
children.push(p('Total active = 20. Plus 10 closed-without-entry = 30.'));
