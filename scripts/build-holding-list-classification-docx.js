...
const bugs = [
  ...
  { id: 'B3a', desc: 'User hears two voices on mobile: Naavi\'s voice + the phone\'s built-in voice', surface: 'both', notes: 'On long replies, the user hears Naavi\'s cloud voice mid-sentence and the phone\'s built-in voice on other parts. The fallback to the phone voice is intentional (so a reply is never silent) but fires too often. Real fix: make the cloud voice reliable enough that fallback rarely fires. Server-side diagnosis pending. Phone-call fragmentation reported separately uses a different audio path and may be a distinct phenomenon.', sa: 'Server' },
  { id: 'B3b', desc: 'Cosmetic ruler leak on long-wrap user bubbles', surface: 'mobile', notes: 'On long-wrap user bubbles, faint dots leak through behind the visible text on Samsung One UI. One-line cosmetic fix in the chat-bubble layout. AAB required.', sa: 'AAB' },
  { id: 'B3c', desc: 'Haptic vibration feels too subtle on Samsung long-press', surface: 'mobile', notes: 'Long-press confirmation buzz (the primary hands-free entry on the chat surface) feels too subtle on Samsung even with system haptics maxed. Bump duration or use a pattern. AAB required; A/B during next AAB cycle.', sa: 'AAB' },
