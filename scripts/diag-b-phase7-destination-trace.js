...
(async () => {
  const res = await fetch(URL, {
    ...
    body: JSON.stringify({ messages: [{ role: 'user', content: 'Drive me to my next meeting' }], max_tokens: 600, user_id: USER_ID, channel: 'app' }),
  });
  const data = await res.json();
  console.log('FULL rawText:', data.rawText);
})();
