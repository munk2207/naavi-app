...
async function search(label, url, key) {
  ...
  console.log(`\n=== ${label} — sent_messages ILIKE '%david%' (last 20) ===`);
  ...
  console.log(`\n=== ${label} — action_rules mentioning 'david' (last 30 rules scanned) ===`);
  ...
  const hits = (rules ?? []).filter(r => JSON.stringify(r).toLowerCase().includes('david'));
