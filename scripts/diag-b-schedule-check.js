...
(async () => {
  const res = await fetch(URL, {
    ...
    body: JSON.stringify({ messages: [{role:'user', content:'What is on my calendar for the next 3 days? List every event with exact time.'}], max_tokens: 900, user_id: USER_ID, channel: 'app' }),
  });
  const data = await res.json();
  const parsed = JSON.parse(data?.rawText ?? '{}');
  console.log('NOW (server-side, from response if present):', data.debug || '(not present)');
