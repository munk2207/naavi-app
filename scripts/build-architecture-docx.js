/**
 * Build docs/ARCHITECTURE_2026-05-13.docx from the matching markdown.
 *
 ...
 *   - Mermaid blocks rendered the same way (as code) — Word can't
 *     paint diagrams, but the source stays readable. For visual
 *     diagrams view the markdown on GitHub instead.
 *
 * Run:  node scripts/build-architecture-docx.js
...
const OUT_PATH = path.join(__dirname, '..', 'docs', 'ARCHITECTURE_2026-05-13.docx');
...
function convertMarkdown(md) {
  ...
  while (i < lines.length) {
    ...
    // Code fence (``` or ```language) — also catches Mermaid blocks.
    if (/^```/.test(line)) {
      const buf = [];
      i++;
    ...
    if (line.includes('|') && /\|/.test(lines[i + 1] ?? '') && /^[\s|:-]+$/.test(lines[i + 1] ?? '')) {
      ...
      while (i < lines.length && lines[i].includes('|')) {
        ...
        // Some tables have pipes at both ends; some don't. Re-split if
        // we ended up with no cells.
    ...
    // Blockquote — render as italic paragraph with a vertical bar look
    // via left-padding (Word doesn't have a great blockquote primitive
    // in this lib; the bold/italic indent reads fine).
    if (/^>\s+/.test(line)) {
...
const doc = new Document({
  creator:  'MyNaavi build-architecture-docx',
  title:    'MyNaavi — Architecture & Wiring (V57.15.0, 2026-05-13)',
  styles: {
    paragraphStyles: [
      {
        id: 'Heading1',
