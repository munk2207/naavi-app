...
(async () => {
  const res = await fetch(URL, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + ANON }, body: JSON.stringify({ messages: [{role:'user', content:'What is on my work list?'}], max_tokens: 600, user_id: USER_ID, channel: 'app' }) });
