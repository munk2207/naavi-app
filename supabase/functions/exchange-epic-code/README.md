# exchange-epic-code — EMPTY ON PURPOSE, AND NEVER BUILT
...
It was meant to exchange the SMART on FHIR authorization code for tokens.
Because it does not exist, `app/auth/epic/callback.tsx` calls an endpoint that
is not there — which is why the OAuth flow could never have completed.
