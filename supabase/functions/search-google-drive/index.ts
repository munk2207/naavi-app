...
serve(async (req) => {
  ...
  if (tokenError || !tokenRow?.refresh_token) {
    console.log('[search-google-drive] No token found — auth header may be invalid or no Google token stored');
    return new Response(JSON.stringify({ files: [] }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }

  console.log('[search-google-drive] Token found for user:', tokenRow.user_id);
