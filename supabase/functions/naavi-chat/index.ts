...
DRAFT_MESSAGE → to_name (recipient name), body (message text), to_phone (E.164 if known), channel ("sms"/"email"/"whatsapp" — STRICT, no guessing: "text"/"SMS"/"message" without the word "email" also present → "sms"; "email" → "email"; "WhatsApp" → "whatsapp"; ambiguous verb ("send"/"write"/"tell") with no channel word → "email"), subject (ONLY when channel="email" — a short, natural 3-7 word subject line derived from the topic/body, e.g. "Budget Review Request", "Meeting Confirmation", "Quick Update" — NEVER just the leftover command words title-cased, e.g. for "Email my wife saying goodnight" write subject:"Goodnight" or subject:"Good Night", NOT "Wife Saying Goodnight". Leave subject empty/omitted for sms/whatsapp — never needed). e.g. "Send sms to my wife saying good morning" → {to_name:"wife",body:"Good morning.",channel:"sms"}. "Text Bob that I'm running late" → {to_name:"Bob",body:"I'm running late.",channel:"sms"}. "Email Sarah asking for the report" → {to_name:"Sarah",body:"Could you please send me the report?",channel:"email",subject:"Report Request"}. "Email my wife saying goodnight" → {to_name:"wife",body:"Goodnight.",channel:"email",subject:"Goodnight"}.
...
function buildActionConfirm(
  ...
  switch (intent) {
    ...
    case 'DRAFT_MESSAGE': {
      ...
      // 2026-08-13 — channel was previously never extracted or set on this
      // fast-path action, so the app's DraftCard defaulted every message to
      // email regardless of what channel word the user said ("send sms" →
      // silently became an email draft). Verified live: 5/5 identical runs
      // of "Send sms to my wife saying good morning" produced an action
      // object with no channel key at all before this fix.
      const draftChannel = ['sms', 'email', 'whatsapp'].includes(String(params.channel ?? '').toLowerCase())
        ? String(params.channel).toLowerCase()
        : 'email';
      const action = { type: 'DRAFT_MESSAGE', to: params.to_name, to_phone: params.to_phone ?? '', body: params.body, subject: draftSubject, channel: draftChannel };
