/**
 * read-drive-file Edge Function
 *
 * Reads the plain-text content of a Google Doc by file ID.
 * Used by the lists feature to retrieve list items.
 *
...
serve(async (req) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ error: `Drive read failed: ${err}` }), {
        status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    const content = await res.text();
    console.log(`[read-drive-file] Read ${fileId} — ${content.length} chars`);
    ...
    console.error('[read-drive-file] Error:', msg);
