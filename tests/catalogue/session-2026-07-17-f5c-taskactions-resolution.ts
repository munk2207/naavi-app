 ...
 * defect (evaluate-rules/index.ts).
 *
 ...
 * block in evaluate-rules took `data.contacts?.[0]` from lookup-contact
 * unconditionally, with no ambiguity check, no confirmation, no fail-closed
 * path. A real incident sent three unconfirmed SMS to wrong real contacts
 * for to_name values "A"/"B"/"C". The fix adds a defense-in-depth
 * name-length guard, replaces the unconditional index-0 pick with an
 * exact-match-count requirement (the actual correctness guarantee), and
 * closes a pre-existing silent-drop logging gap.
 *
...
export const session2026_07_17_f5cTaskActionsResolutionTests: TestCase[] = [
  {
    ...
    category: 'rules',
    description: 'F5c block rejects a to_name under 2 characters (defense-in-depth) before ever calling lookup-contact, and logs the reason as name_too_short',
    async run() {
      ...
      expectTruthy(guardIdx > -1, 'name-length guard must exist in the F5c block');
  ...
  },
  {
    ...
    category: 'rules',
    description: 'the correctness guarantee — F5c resolves a task_action only when lookup-contact returns exactly one match; the prior unconditional data.contacts?.[0] pick is gone',
    async run() {
      const src = readFileSync(EVALUATE_RULES_PATH, 'utf8');
      expectTruthy(
        src.includes('matches.length === 1'),
        'F5c must require exactly one match before resolving a recipient',
      );
      expectFalsy(
  ...
  },
  {
    ...
    category: 'rules',
    description: 'regression guard — the primary self/third-party alert fan-out logic (outside the F5c block) is untouched by this fix; the F5c block still runs after it, per the function\'s own existing ordering comment',
    async run() {
      const src = readFileSync(EVALUATE_RULES_PATH, 'utf8');
      const fanoutModeIdx = src.indexOf("const mode = isSelfAlert ? 'self' : (toPhone ? 'third-party-phone' : 'third-party-email');");
      const f5cCommentIdx = src.indexOf('// F5c — execute structured task_actions stored at alert-set time.');
      expectTruthy(fanoutModeIdx > -1, 'primary fan-out mode logic must still exist, unchanged');
      expectTruthy(f5cCommentIdx > -1, 'F5c block header comment must still exist');
      expectTruthy(
        fanoutModeIdx < f5cCommentIdx,
        'the primary alert fan-out must still run before the F5c task_actions block (ordering unchanged by this fix)',
      );
    },
  },
];
