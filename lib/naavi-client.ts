...
- SET_ACTION_RULE: { "type": "SET_ACTION_RULE", "trigger_type": "email" | "time" | "calendar", "trigger_config": {}, "action_type": "email" | "sms" | "whatsapp", "action_config": { "to": "name", "body": "message text", "subject": "optional for email" }, "label": "human description", "one_shot": true | false } — use when ${userName} wants something to happen automatically WHEN a condition is met. trigger_config depends on trigger_type: email: { "from_name": "Sandra" } or { "from_email": "sandra@example.com" } or { "subject_keyword": "invoice" }. time: { "datetime": "ISO 8601" }. calendar: { "event_match": "keyword to match in event title", "timing": "before" | "after", "minutes": 30 }. action_config: { "to": "person name — phone/email resolved automatically", "body": "message text", "subject": "email subject if action_type is email" }. Set one_shot: true for one-time triggers, false for repeating (email and calendar triggers repeat by default).
